...
def retrievePath():
    ...
    try:
        ...
        start_pose.pose.position.x = -2.67
        start_pose.pose.position.y = 2.15
        start_pose.pose.position.z = -0.16

        goal_pose = PoseStamped()
        goal_pose.header.stamp = rospy.Time.now()
        goal_pose.header.frame_id = "map"
        goal_pose.pose.position.x = -2.67
        goal_pose.pose.position.y = 2.15
        goal_pose.pose.position.z = -0.16
...
def parseValues(axis):  
    ...
    try:
        with open("/home/ubuntu/ethz_catkin_ws/src/ETHZ/maps/arena_complex/" + axis + "_coordinates.txt") as file_reader:
        ...
        if(response.success):
            ...
            if(response.success):
                for point in response.sampled_plan.points:
                    sendGoal(point)
                #sendGoal(response.sampled_plan.points[0])
                #sendGoal(response.sampled_plan.points[1])
                #sendGoal(response.sampled_plan.points[2])
